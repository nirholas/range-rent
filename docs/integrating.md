# Integrating RangeRent

## From Solidity

```solidity
import {RangeRentHook} from "range-rent/src/hooks/RangeRentHook.sol";
...
hook.configure(key, RangeRentHook.Config({ /* see the README */ }));
...
const hook = getHook("range-rent");
const address = hookAddress("range-rent", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "RangeRent"
...
More at https://range-rent.pages.dev.
