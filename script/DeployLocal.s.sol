...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(RangeRentHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new RangeRentHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, RangeRentHook.Config({rangeWidth: 600, rentPerSecond: 1e12, minTerm: 1 hours, maxTerm: 7 days}));
