...
/**
 * @title DeployRangeRent
 * @notice Deploys RangeRent deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployRangeRent is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(RangeRentHook).creationCode;
        ...
        console2.log("RangeRent ", predicted);
        ...
        vm.startBroadcast();
        RangeRentHook hook = new RangeRentHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("RangeRent")),
